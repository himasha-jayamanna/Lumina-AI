"""
Enterprise Policy Copilot - Production-Ready Multi-Agent RAG Backend
...
logger = logging.getLogger("enterprise_copilot")
...
def get_embeddings():
    ...
    if _embeddings is None:
        ...
        if api_key and not use_local:
            logger.info("Initializing Google Generative AI Embeddings (gemini-embedding-2)...")
            ...
            logger.info(f"Initializing local HuggingFace embeddings ({embed_model})...")
...
def get_llm(model_name: str = "gemini-2.5-flash"):
    """Returns a dynamic model based on the available provider."""
    openrouter_api_key = os.getenv("OPENROUTER_API_KEY")
    api_key = os.getenv("GEMINI_API_KEY") or os.getenv("GOOGLE_API_KEY")
    
    if openrouter_api_key:
        openrouter_model = os.getenv("OPENROUTER_MODEL", "meta-llama/llama-3.3-70b-instruct:free")
        logger.info(f"Connecting to OpenRouter (Free Tier) | Model: {openrouter_model}...")
        ...
        if "gemini-2.5-flash" in model_name or "gemini-1.5-flash" in model_name:
            mapped_model = "gemini-2.0-flash"
        logger.info(f"Connecting to Google AI Studio (Free Cloud Tier) | Model requested: {model_name} -> Mapped: {mapped_model}...")
        ...
        logger.info(f"GEMINI_API_KEY and OPENROUTER_API_KEY not found. Connecting to local Ollama | Model: {ollama_model}...")
...
MSSQL_DATABASE = os.getenv("MSSQL_DATABASE", "Enterprise_Copilot_DB")
...
def communicator_node(state: AgentState) -> AgentState:
    ...
    COMPANY_FACT_SHEET = (
        ...
        "- Company: Enterprise Finance Corporation (EFC) PLC\n"
        "- Founder / Owner / Visionary: Dr. J. D. Wayne (Founder of the Enterprise Movement)\n"
        "- Board of Directors:\n"
        "  * Mr. George Smith (Chairman/Non-Executive, Non-Independent Director)\n"
        "  * Mr. James Ward (Senior Director/Non-Executive, Independent Director)\n"
        "  * Mr. Arthur Pendelton (Non-Executive, Independent Director)\n"
        "  * Mr. Andrew Vance (Non-Executive, Non-Independent Director)\n"
        "  * Ms. Jane Sterling (Non-Executive, Independent Director)\n"
        "  * Ms. Susan Higgins (Non-Executive, Independent Director)\n"
        "  * Mr. Henry Sterling (Non-Executive, Independent Director)\n"
        "  * Ms. Beatrice Vance (Non-Executive, Non-Independent Director)\n"
        "  * Mr. David Ross (Non-Executive, Non-Independent Director)\n"
        "- Corporate Management Team:\n"
        "  * Mr. Robert Miller (Chief Executive Officer - CEO)\n"
        "  * Mr. Richard Taylor (Chief Operating Officer - COO)\n"
        "  * Mr. Raymond Martinez (Head of Credit)\n"
        "  * Mr. Mark Johnson (Head of Strategic Planning)\n"
        "  * Ms. Michelle Watson (Head of Finance)\n"
        "  * Mr. Kevin Green (Head of Operations & Administration)\n"
        "  * Mr. Ian Davis (Head of Information Technology - IT)\n"
        "  * Mr. Ryan Harris (Head of Human Resources - HR)\n"
        "  * Mr. Kenneth Thompson (Head of National Sales)\n"
        "  * Mr. Patrick Robinson (Head of Gold Loan)\n"
        "  * Ms. Patricia Clark (Head of Risk Management)\n"
        "  * Mr. Matthew Anderson (Head of Compliance)\n"
        "  * Mr. Ronald Cooper (Head of Recovery)\n"
        "  * Mr. Alan White (Head of Internal Audit)\n"
        "  * Ms. Margaret Baker (Head of Legal)\n"
        "  * Ms. Irene Walker (Company Secretary)\n"
    )

    prompt = (
        "You are a Professional Corporate Communications AI for Enterprise Finance Corporation (EFC). "
        ...
        "3. IF CONTEXT is 'NO NEW DOCUMENTS RELEVANT' AND the user is asking a long, general question NOT related to history, politely explain that you are the Enterprise Policy Agent and can only answer questions based on official internal documents.\n"
...
async def stream_chat(request: ChatRequest):
        ...
        if conn:
            try:
                ...
                if is_greeting:
                    ...
                    greeting = "I am the Enterprise Policy Agent, an enterprise AI assistant designed to help you strictly with Enterprise Finance Corporation's internal documents and HR policies."
                elif is_smalltalk:
                    greeting = f"I'm doing great, {user_name}! Thanks for asking. How can I assist you with Enterprise policies today?"
                elif is_gibberish:
                    greeting = "Please ask a specific question related to Enterprise Finance Corporation's internal documents or policies."
            ...
            elif is_identity: greeting = "I am the Enterprise Policy Agent, an enterprise AI assistant designed to help you strictly with Enterprise Finance Corporation's internal documents and HR policies."
            elif is_smalltalk: greeting = "I'm doing great! Thanks for asking. How can I assist you with Enterprise policies today?"
            elif is_gibberish: greeting = "Please ask a specific question related to Enterprise Finance Corporation's internal documents or policies."
    ...
    if conn:
        try:
            cursor = conn.cursor()
            cursor.execute("SELECT Department, Role FROM Accounts WHERE Username = ?", request.employee_id)
            ...
            user_email = request.employee_id.strip().lower()
...
GROUP_TO_DEPT_MAPPING = {
    "mancom@enterprise.com": "MANCOM",
    "compliancedepartment@enterprise.com": "COMPLIANCE TEAM",
    "auditdepartment@enterprise.com": "INTERNAL AUDIT",
    "risk@enterprise.com": "RISK MANAGEMENT",
    "boardofdirectors@enterprise.com": "BOARD",
    "tencomm@enterprise.com": "TENDER COMMITTEE",
    "finance@enterprise.com": "FINANCE",
    "creditdepartment@enterprise.com": "CREDIT",
    "recoverydepartment@enterprise.com": "RECOVERY",
    "alco@enterprise.com": "ALCO",
    "cau@enterprise.com": "CAU",
    "itsecurity@enterprise.com": "ITSC",
    "itgroup@enterprise.com": "IT",
    "operationsdepartment@enterprise.com": "OPERATIONS",
    "productdevelopmentcommittee@enterprise.com": "PDC",
    "productheads@enterprise.com": "PRODUCT HEADS",
    "sustainabilitycommittee@enterprise.com": "SUSTAINABILITYCOMMITTE",
    "ormc@enterprise.com": "ORMC",
    "goldloandepartment@enterprise.com": "GOLD LOAN",
    "humanresources@enterprise.com": "HR",
    "legaldepartment@enterprise.com": "LEGAL",
    "marketingdepartment@enterprise.com": "MARKETING"
}
...
async def google_sso(req: Dict[str, str]):
    """Google SSO - Verify Google token and auto-login/create Enterprise employees."""
    try:
...
async def list_user_docs(username: str):
    ...
    try:
        cursor = conn.cursor()
        cursor.execute("SELECT Department, Role FROM Accounts WHERE Username = ?", username)
        user_row = cursor.fetchone()
        user_dept_str = (user_row[0] or "").strip() if user_row else ""
        user_depts = [d.strip() for d in user_dept_str.split(",")] if user_dept_str else []
        user_email = username.strip().lower() # Fallback to username for email logic
        user_role = user_row[1] if user_row else "user"
