...
**Enterprise Policy Assistant** is a production-grade AI assistant that enables Enterprise Finance Corporation (EFC) employees to query internal company policy documents through a secure, conversational interface.

Employees log in with their official `@enterprise.com` Google account (SSO). The AI reads uploaded policy PDFs, understands questions in plain language, and responds with accurate, cited answers — verified by a multi-agent hallucination-checking pipeline.
...
| **Authentication** | Google SSO (`@enterprise.com` accounts) |
...
│  │  Vector Store│  │  text-embedding   │  │  Enterprise_Copilot│  │
│  └──────────────┘  └───────────────────┘  └─────────────┘  │
└─────────────────────────────────────────────────────────────┘
```
...
| Process Manager | PM2 |
| Domain | `chatbot.staging.enterprise.com` |
...
│   ├── Enterprise_AI_assistent_Developer_Guide.md
...
# Restart frontend
pm2 restart enterprise-frontend
...
|---|---|---|---|
| `enterprise-backend` | 8000 | 20 | `uvicorn main:app --host 0.0.0.0 --port 8000 --workers 20` |
| `enterprise-frontend` | 5173 | 1 | `npm run dev -- --port 5173 --host` |

### Nginx Config Location
```
/etc/nginx/sites-available/enterprise-chatbot
```
...
Base URL (Production): `https://chatbot.staging.enterprise.com/api`  
...
Authentication is handled via **Google OAuth 2.0**. Only `@enterprise.com` email addresses are permitted.
...
| Issue | Cause | Fix |
|---|---|---|
| `502 Bad Gateway` | Backend not running | `pm2 restart enterprise-backend` |
...
pm2 start npm --name "enterprise-frontend" -- run dev -- --port 5173 --host
...
[`documemnt/Enterprise_AI_assistent_Developer_Guide.md`](./documemnt/Enterprise_AI_assistent_Developer_Guide.md)
...
*Enterprise Policy Assistant — Enterprise Finance Corporation PLC*  
*Developed by Enterprise IT Development Team · 2026*
