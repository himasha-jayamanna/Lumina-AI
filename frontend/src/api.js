...
export async function getUserDocuments(username) {
  const { data } = await API.get(`/user/documents/${encodeURIComponent(username)}`);
  return data;
}
...
export async function deleteDocument(filename, admin_id) {
  const { data } = await API.delete(`/admin/document/${encodeURIComponent(filename)}?admin_id=${admin_id}`);
  return data;
}
...
export async function deleteAccount(username, admin_id) {
  const { data } = await API.delete(`/admin/account/${encodeURIComponent(username)}?admin_id=${admin_id}`);
  return data;
}

export async function updateAccount(username, role, name, preferred_name, emp_num, department, password, admin_id) {
  const { data } = await API.put(`/admin/account/${encodeURIComponent(username)}`, { role, name, preferred_name, emp_num, department, password, admin_id });
  return data;
}
